...
- [ ] **Battery: unresolved, needs a multimeter.** Daily batching worked (nginx confirms 4 POSTs/day, down from 24/board) but battery life on DIY MORE stayed ~7 days. WiFi was only ~3% of the power budget; the remaining ~11.5mA constant draw is hardware (AMS1117 quiescent + power LED + possibly always-on sensors). The historical "16-day runs" used as a baseline turned out to be outages, not battery cycles — all four boards stopped/resumed in lockstep. Cells were also 6 months old with a 4-month shelf period. **Next step: measure deep-sleep current, USB disconnected, NodeMCU vs DIY MORE side by side.** Full write-up in `docs/battery-investigation.md`.
