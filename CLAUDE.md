...
- [x] Firmware versioning — every sketch defines `FIRMWARE_VERSION` and sends it as `fw_version` in the POST payload. Backend stores it on `sensors.firmware_version` and logs changeovers to `firmware_history` (migration 004), so battery life / behaviour can be attributed to a specific build. **Bump `FIRMWARE_VERSION` on any change that could affect field behaviour.** Current: all three sketches at `2.1.0`.
- [x] Firmware 2.1.0 robustness pass — cold-boot immediate upload (a reconnected board confirms itself in ~30s instead of looking dead for 24h), exponential upload backoff (a failed upload no longer retries every wake — 2/4/8 wakes, batch dropped after 4 failures), ESP8266 `RF_DISABLED` on non-upload wakes (the radio otherwise powers up every wake even when unused), ESP32 GPIO hold in deep sleep (stops phantom-powering gated sensors through their signal pins).
- [x] NodeMCU (Ms Green, 8C:CE:4E:CE:66:15) reflashed to **2.1.0** and back online 2026-09-22 19:16 UTC after 5 months dark. Confirms in the field: version reporting works (`sensors.firmware_version` = 2.1.0), cold-boot immediate upload works (sent a single reading set on first wake, not a 24h batch), and the HTU21D is healthy again (temp 27.4C + humidity 43.6% both valid, previously stuck at 999/CRC error). The 4 DIY MORE boards are also back online as of 2026-09-21, still on 2.0.0 (no `fw_version` reported).
- [ ] **Battery: unresolved, needs a multimeter.** Daily batching worked (nginx confirms 4 POSTs/day, down from 24/board) but battery life on DIY MORE stayed ~7 days. WiFi was only ~3% of the power budget; the remaining ~11.5mA constant draw is hardware (AMS1117 quiescent + power LED + possibly always-on sensors). The historical "16-day runs" used as a baseline turned out to be outages, not battery cycles — all four boards stopped/resumed in lockstep. Cells were also 6 months old with a 4-month shelf period. **Next step: measure deep-sleep current, USB disconnected, NodeMCU vs DIY MORE side by side.** Attempt on 2026-09-22 failed on the meter, not the board: uA range can't pass the boot inrush (board browns out, LED dead), and mA showed nothing either - likely a blown meter fuse. Use a 10 ohm resistor in series with the cell and measure the voltage drop in DC volts mode instead (I = U/R; 110mV = 11mA = hardware mod needed, 2mV = 0.2mA = firmware fine). Cell measured 4.052 V (healthy, near full). Full write-up in `docs/battery-investigation.md`.
...
- [x] Hardware design spec — `docs/hardware-design.md`. Purpose-built sensor node replacing the prebuilt DIY MORE boards, grounded in this project's actual failures: MCP1700 instead of AMS1117 (the ~4000x quiescent difference that explains 7-day vs 60-day life), no power LED, no populated USB-serial chip, every sensor rail switched via P-MOSFET, potted M8 cable entry + ePTFE vent + IP54 body (the Ms Green node was lost to water ingress after a watering), and a battery-voltage divider (its absence is why a flat cell and a WiFi outage were indistinguishable this month). Budget ~1.0 mAh/day circuit draw; realistic life ~600-700 days because lithium self-discharge (~2 mAh/day) then dominates. Includes BOM (~£16/node) and a solder-iron validation path before committing to a PCB spin.
- [ ] Google Home (future)
